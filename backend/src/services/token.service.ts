...
export class TokenService {
    ...
    generateTokens(payload: PayloadUser): {accessToken: string, refreshToken: string} {
        const accessToken = jwt.sign(payload, 'jwt_access_JOIjii)(98', {
            expiresIn: "1d",
        });
        const refreshToken = jwt.sign(payload, 'jwt_refresh_KKL*&*89ds3', {
            expiresIn: "30d",
        });
        return {
            accessToken,
    ...
    validateAccessToken(token: string) {
        try {
            const userData = jwt.verify(token, 'jwt_access_JOIjii)(98')  as PayloadUser;
            return userData;
        } catch(e) {
            throw e;
        }
    ...
    validateRefreshToken(token: string)  {
        try {
            const userData = jwt.verify(token, 'jwt_refresh_KKL*&*89ds3') as PayloadUser;
            return userData;
        } catch(e) {
            throw e;
        }
